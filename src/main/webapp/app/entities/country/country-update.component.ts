...
export default defineComponent({
  ...
  setup() {
    ...
    const retrieveCountry = async countryIsoCode => {
      try {
        const res = await countryService().find(countryIsoCode);
        country.value = res;
      } catch (error) {
      ...
      if (this.country.isoCode) {
        this.countryService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('mobilePosApp.country.updated', { param: param.isoCode }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.countryService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('mobilePosApp.country.created', { param: param.isoCode }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
