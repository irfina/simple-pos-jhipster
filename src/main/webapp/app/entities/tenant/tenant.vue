<template>
  <div>
    <h2 id="page-heading" data-cy="TenantHeading">
      <span v-text="t$('mobilePosApp.tenant.home.title')" id="tenant-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('mobilePosApp.tenant.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'TenantCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('mobilePosApp.tenant.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && tenants && tenants.length === 0">
      <span v-text="t$('mobilePosApp.tenant.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="tenants">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('mobilePosApp.tenant.name')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="mobilePosApp.tenant.delete.question" data-cy="tenantDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-tenant-heading" v-text="t$('mobilePosApp.tenant.delete.question', { id: removeId })"></p>
      </div>
