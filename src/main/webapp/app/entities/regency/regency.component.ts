...
export default defineComponent({
  ...
  setup() {
    ...
    const regencies: Ref<IRegency[]> = ref([]);
    ...
    const retrieveRegencys = async () => {
      ...
      try {
        ...
        const res = await regencyService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.['link']);
        regencies.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    onMounted(async () => {
      await retrieveRegencys();
    });

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: IRegency) => {
    ...
    const removeRegency = async () => {
      try {
        await regencyService().delete(removeId.value);
        const message = t$('mobilePosApp.regency.deleted', { param: removeId.value }).toString();
    ...
    watch([regencies, page], async ([data, page], [_prevData, prevPage]) => {
