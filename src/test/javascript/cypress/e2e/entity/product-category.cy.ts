...
describe('ProductCategory e2e test', () => {
  ...
  const productCategorySample = { code: 'stick next', name: 'however difficult' };
  ...
  describe('new ProductCategory page', () => {
    ...
    it('should create an instance of ProductCategory', () => {
      cy.get(`[data-cy="code"]`).type('woefully');
      cy.get(`[data-cy="code"]`).should('have.value', 'woefully');

      cy.get(`[data-cy="name"]`).type('till caramelize');
      cy.get(`[data-cy="name"]`).should('have.value', 'till caramelize');

      cy.get(`[data-cy="notes"]`).type('shootdown');
      cy.get(`[data-cy="notes"]`).should('have.value', 'shootdown');
